...
func _check_payload_is_sanitized(page) -> void:
	...
	assert(system_content.contains("user prompt 是一个 JSON 对象字符串，字段包括："))
	assert(system_content.contains("current_question 当前需要回答的问题。"))
	assert(system_content.contains("targetOptions 当前动作可选的合法目标列表"))
	assert(system_content.contains("players 当前视角可见的玩家列表"))
	...
	var user_json = JSON.parse_string(user_content)
	assert(user_json is Dictionary)
	assert(not (user_json as Dictionary).has("schema"))
	assert(not (user_json as Dictionary).has("visibleState"))
	assert((user_json as Dictionary).has("targetOptions"))
	assert((user_json as Dictionary).get("timeline", []) is Array)
...
func _check_kimi_robot_model_request_payload(page, context: Dictionary, action_messages: Array, response_schema: Dictionary) -> void:
	...
	assert(speech_system.contains("建议发言控制在120字以内"))
...
func _check_guard_target_options(page) -> void:
	...
	var payload: Dictionary = page._bot_runtime.to_model_payload(context)
	assert(payload.has("targetOptions"))
	assert(String(payload.get("current_question", "")).contains("上一夜已守护2号"))
	var targets: Array = page._bot_runtime.target_options_for_context(context)
	assert(targets.size() == 1)
...
func _check_sheriff_vote_self_target(page) -> void:
	...
	var payload: Dictionary = page._bot_runtime.to_model_payload(context)
	assert(payload.has("targetOptions"))
	assert(String(payload.get("current_question", "")).contains("可投自己"))
	var targets: Array = page._bot_runtime.target_options_for_context(context)
...
func _check_sheriff_speech_order_context(page) -> void:
	...
	var payload: Dictionary = page._bot_runtime.to_model_payload(context)
	assert(payload.has("targetOptions"))
	assert(String(payload.get("current_question", "")).contains("第一位发言"))
	assert(String(payload.get("current_question", "")).contains("顺时针"))
	assert(String(payload.get("current_question", "")).contains("逆时针"))
	var messages: Array = page._bot_runtime.build_messages(context)
	var system_content := String((messages[0] as Dictionary).get("content", ""))
	assert(system_content.contains("[警长发言顺序]"))
...
func _check_sheriff_badge_context(page) -> void:
	...
	var messages: Array = page._bot_runtime.build_messages(context)
	var system_content := String((messages[0] as Dictionary).get("content", ""))
	assert(system_content.contains("[警徽移交]"))
...
func _check_witch_skip_and_save(page) -> void:
	...
	var payload: Dictionary = page._bot_runtime.to_model_payload(context)
	assert(String(payload.get("current_question", "")).contains("今晚被袭击的是2号"))
	assert(payload.has("targetOptions"))
	var targets: Array = page._bot_runtime.target_options_for_context(context)
	assert(targets.size() == 1)
	assert(int((targets[0] as Dictionary).get("targetSeatNumber", -1)) == 2)
