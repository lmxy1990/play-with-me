...
创建房间模块是小编织模块，负责读取可创建的游戏房间模块列表、每个游戏房间模块提供的地图列表，以及每张地图支持的人数列表，并据此提供统一创建 UI。UI 上把游戏房间模块展示为“玩法”下拉；选择玩法后，地图、人数、场景槽位和玩法专属配置跟随变化。它把用户选择的玩法、地图、人数、密码、观战策略和玩法专属配置整理成创建请求，交给房间模块完成后续建房、开主机和发布发现，再根据返回结果路由进入房间。
...
2. 表单状态层：保存当前选择的玩法、地图、席位、密码、观战参数和玩法专属配置。
...
`game_room_init_options` 由当前玩法决定：

| 玩法 | 初始化选项 |
| --- | --- |
| 狼人杀 | 地图和人数决定身份配置预览；创建模块只提交房间模块允许的规则引用或显式选项。 |
| 象棋 | `clock_enabled` 默认 `false`；开启后显示时间下拉并提交 `time_limit_ms`，默认 600000；求和、悔棋、认输第一版固定启用。 |
...
- 切换玩法时必须重置为该玩法的默认地图、默认人数、默认场景槽位和默认 `game_room_init_options`。
- `password` 只做格式和长度校验，不在这里做房间认证。
- `allow_observers` 和 `max_observers` 必须彼此一致。
- 玩法专属配置必须由所选游戏房间模块校验；创建模块只做 UI 层格式和范围校验。
...
| `game_room_init_options` | 当前玩法专属配置。 |
| `preview` | 创建预览。 |
...
| `game_room_init_summary` | 玩法专属配置摘要，例如象棋是否计时和每方时长。 |
